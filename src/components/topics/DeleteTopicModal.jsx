      ...
      >
        ...
        <div className="p-6 overflow-y-auto space-y-3 flex-1">
          {subjectTopics.length === 0 ? (
            <p className="text-slate-400 text-center py-4">No topics to delete.</p>
          ) : (
            <>
              <div className="flex items-start gap-3 p-3 bg-red-500/10 border border-red-500/20 rounded-lg mb-4">
                <AlertTriangle className="w-5 h-5 text-red-400 shrink-0 mt-0.5" />
                <p className="text-xs text-red-300">
                  Deleting a topic will permanently remove all of its tasks. This action cannot be undone.
                </p>
              </div>
              {subjectTopics.map((topic) => (
                <div key={topic.slug} className="flex items-center justify-between p-3 bg-slate-800 rounded-lg border border-slate-700">
                  <span className="text-slate-200 font-medium truncate pr-4">{topic.name}</span>
                  <button
                    onClick={() => {
